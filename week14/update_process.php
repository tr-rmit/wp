...
// Step 2.1: Clean up POST data
  foreach ($_POST as $name => $value) {
    $$name = htmlentities(trim($value));
  }      

// 3. If there is a new image we need to perform checks
  if (!empty($_FILES['image']['name'])) {
    if($_FILES['image']['error']!=0) {
      die('file upload error');
    } else if(!preg_match("#^image/#",$_FILES['image']['type'])) {
      die('not an (allowed) image');
    } else if($_FILES['image']['size'] > 1000000) {
      die('image too big!');
    }
    // 3.1. new image is ok! 
    $newImageName = htmlentities(trim($_FILES['image']['name']));
    $tmpImageName = htmlentities(trim($_FILES['image']['tmp_name']));
  }  

  // 4. Read the existing record in the database to get old image name
...
// 5. At this stage, we have no image OR enough image details needed to perform an update, let's start creating a HTML page
...
?>
  <main class="container">
    <h1><?= $title ?></h1>
<?php
// 6. This is a bit of a hack: we are not updating the image name if it doesn't exist, but rather than write two separate SQL statements, we will use the old image name in an update if a new image name does not exist
